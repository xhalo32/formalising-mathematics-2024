...
-- I.e. filters are closed under finite intersection
example : S ∩ T ∈ F ↔ S ∈ F ∧ T ∈ F := by
  constructor
  · intro h
    constructor
    · exact mem_of_superset h (inter_subset_left _ T)
    · exact mem_of_superset h (inter_subset_right _ T)
  · intro h
    exact inter_mem h.1 h.2

-- I.e. filters are closed under arbitrary non-empty unions
example (S : Set (Set α)) (hs : Nonempty S) : (∀ s ∈ S, s ∈ F) → ⋃₀ S ∈ F := by
  intro h
  cases' hs with s
